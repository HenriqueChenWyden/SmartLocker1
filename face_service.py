...
IMG_SIZE = (100, 100)
CONFIDENCE_THRESHOLD = float(os.getenv("CONFIDENCE_THRESHOLD", "100.0"))
...
def save_user_image(user: str, file_bytes: bytes, filename: str = None) -> str:
    ...
    # Save image through storage backend
    path = storage.save_image(user, filename, file_bytes)
    # Invalidate cache so next recognize reloads updated models
...
def recognize_image_bytes(img_bytes: bytes) -> Dict:
    ...
    if best["confidence"] < CONFIDENCE_THRESHOLD:
        ...
        return {"found": False, "confidence": float(best["confidence"]), "reason": "low-confidence"}
